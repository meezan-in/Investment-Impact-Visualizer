...
const dealsData: Deal[] = [
  {
    ...
    progress: 50,
    description: "Expanding craft beer operations with focus on sustainable brewing practices and local ingredient sourcing."
  },
  {
    id: 2,
    name: "Asha Textiles",
    ...
    progress: 82,
    description: "Modernizing textile production with eco-friendly processes and automation technology."
  },
  {
    id: 3,
    ...
    progress: 32,
    description: "Opening new specialty coffee outlets across Bengaluru with focus on premium blends and artisanal brewing."
  },
  {
    id: 4,
    ...
    progress: 78,
    description: "Developing AI-powered solutions for small and medium enterprises in Bengaluru's tech ecosystem."
  },
  {
    id: 5,
    ...
    progress: 50,
    description: "Traditional spice processing and packaging with modern quality control systems."
  },
  {
    id: 6,
    ...
    progress: 50,
    description: "Developing smart transportation solutions for urban commuters in Bengaluru."
  },
  {
    id: 7,
    ...
    targetFunding: 500000,
    progress: 40,
    description: "Sustainable fashion retail focusing on eco-friendly materials and ethical manufacturing."
  },
  {
    id: 8,
    ...
    progress: 45,
    description: "Traditional handicrafts and artisan products with modern marketing and distribution."
  }
];
...
const Home: React.FC = () => {
  ...
  useEffect(() => {
    const checkNetworkConnection = () => {
      console.log('🌐 Network Information API: Checking connection...');
      if ('connection' in navigator) {
        const connection = (navigator as any).connection;
        if (connection) {
          const effectiveType = connection.effectiveType;
          const downlink = connection.downlink;
          console.log(`🌐 Network Information API: Type: ${effectiveType}, Speed: ${downlink}Mbps`);
          
          if (effectiveType === 'slow-2g' || effectiveType === '2g' || downlink < 1) {
            setIsLowDataMode(true);
            document.body.classList.add('no-animation');
            console.log('🌐 Network Information API: Low data mode enabled');
          }
          
          // Listen for network changes
          connection.addEventListener('change', () => {
            console.log('🌐 Network Information API: Connection changed');
    ...
    const getUserLocation = () => {
      ...
      navigator.geolocation.getCurrentPosition(
        (position) => {
          ...
          console.log(`📍 Geolocation API: Location found - Lat: ${userLat}, Lng: ${userLng}, Accuracy: ${accuracy}m`);
  ...
  useEffect(() => {
    const initializeMap = async () => {
      ...
      try {
        ...
        // Add deal markers
        deals.forEach(deal => {
          const marker = L.marker(deal.coordinates, { icon: dealIcon }).addTo(map);
          
          // Create popup content
          const popupContent = `
            <div class="p-2 min-w-[200px]">
              <h3 class="font-bold text-lg mb-1">${deal.name}</h3>
              <p class="text-sm text-gray-600 mb-2">${deal.sector} • ${deal.location}</p>
              <div class="mb-2">
                <div class="text-xs text-gray-500">Funding Progress</div>
                <div class="w-full bg-gray-200 rounded-full h-2">
                  <div class="bg-[#2DD4BF] h-2 rounded-full" style="width: ${deal.progress}%"></div>
                </div>
                <div class="text-xs text-gray-600 mt-1">${deal.progress}% (₹${(deal.currentFunding/100000).toFixed(1)}L / ₹${(deal.targetFunding/100000).toFixed(1)}L)</div>
              </div>
        ...
        if (canvasRef.current) {
          ...
          if (ctx) {
            ...
            ctx.fillText('Map temporarily unavailable', canvasRef.current.width / 2, canvasRef.current.height / 2);
          }
        }
      }
    };
  ...
  useEffect(() => {
    ...
    const resizeCanvas = () => {
      if (mapRef.current) {
        ...
        console.log(`🎨 Canvas API: Canvas resized to ${canvas.width}x${canvas.height}`);
      }
    };
    ...
    const animateMarkers = () => {
      ...
      if (activeDeal && mapInstanceRef.current) {
        try {
          ...
          const point = mapInstanceRef.current.latLngToContainerPoint(activeDeal.coordinates);
          ...
          ctx.arc(point.x, point.y, 25, -Math.PI / 2, -Math.PI / 2 + (progress * 2 * Math.PI));
          ...
          console.log(`🎨 Canvas API: Drew marker at (${point.x}, ${point.y}) for ${activeDeal.name}`);
        } catch (error) {
  ...
  useEffect(() => {
    ...
    console.log(`👁️ Intersection Observer API: Observing ${dealCards.length} deal cards`);
    ...
    const observer = new IntersectionObserver(
      (entries) => {
        ...
        entries.forEach(entry => {
          ...
          console.log(`👁️ Intersection Observer API: Deal ${dealId} visibility: ${(ratio * 100).toFixed(1)}%`);
        ...
        if (mostVisibleDeal && highestRatio > 0.6) {
          const dealId = parseInt(mostVisibleDeal.getAttribute('data-deal-id') || '0');
          if (dealId && dealId !== activeDealId) {
            console.log(`👁️ Intersection Observer API: Activating deal ${dealId} (${(highestRatio * 100).toFixed(1)}% visible)`);
            setActiveDealId(dealId);
          }
        }
      },
  ...
  const handleInvestmentSubmit = useCallback(() => {
    if (investmentAmount && selectedDeal) {
      ...
      alert(`Investment of ₹${investmentAmount} in ${selectedDeal.name} submitted successfully!`);
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <section className="mb-12 text-center">
          ...
          <h2 className="text-2xl lg:text-3xl font-semibold mb-8 text-gray-300">
            Our Solution: We make your investments tangible by visually connecting you to the real-world Bengaluru businesses you're empowering.
          </h2>
          <div className="glass-morphism p-8 rounded-2xl max-w-4xl mx-auto">
            <p className="text-lg text-gray-300 leading-relaxed">
              Experience the future of investment transparency. See exactly where your money goes, 
              track the real businesses you're supporting, and witness the tangible impact of your 
              financial decisions in Bengaluru's thriving ecosystem.
            </p>
          </div>
        </section>
          ...
          <div className="space-y-6">
            ...
            <Card className="glass-morphism border-white/10">
              <CardContent className="p-6">
                ...
                <div ref={dealsContainerRef} className="space-y-6 max-h-96 overflow-y-auto pr-2">
                      ...
                      {filteredAndSortedDeals.map((deal) => (
                        ...
                        >
                          <div className="flex items-start justify-between mb-3">
                            <div>
                              <h4 className="font-semibold text-lg text-white">{deal.name}</h4>
                              <p className="text-sm text-gray-400">{deal.sector} • {deal.location}</p>
                            </div>
                            <div className="text-right">
                              <div className="text-[#2DD4BF] font-semibold">{formatCurrency(deal.currentFunding)}</div>
                              <div className="text-xs text-gray-400">Target: {formatCurrency(deal.targetFunding)}</div>
                            </div>
                          </div>
                          <div className="mb-3">
                            <div className="flex justify-between text-sm mb-1">
                              <span className="text-white">Funding Progress</span>
                              <span className="text-[#2DD4BF]">{deal.progress}%</span>
                            </div>
                          ...
                          <div className="flex items-center justify-between">
                            <div className="flex items-center space-x-2">
                              ...
                              <span className="text-xs text-gray-400">
                                {deal.coordinates[0].toFixed(4)}° N, {deal.coordinates[1].toFixed(4)}° E
                              </span>
                            </div>
                            <Button 
                              size="sm" 
              ...
              <CardContent className="p-4">
                ...
                <div className="space-y-2">
                  <div className="flex items-center space-x-3">
                    ...
                    <span className="text-sm text-white">Active Investment</span>
                  </div>
                  <div className="flex items-center space-x-3">
                    <div className="w-4 h-4 bg-[#8B5CF6] rounded-full" />
                    <span className="text-sm text-white">Available Opportunity</span>
                  </div>
                  <div className="flex items-center space-x-3">
      ...
      <footer className="glass-morphism mt-16 py-12">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {/* APIs Used Section */}
            <div>
              <h4 className="text-lg font-semibold mb-4 text-white">APIs Used in this Project</h4>
              <div className="space-y-3">
                <div className="flex items-center justify-between">
                  ...
                  <div className="w-2 h-2 bg-green-500 rounded-full animate-pulse" title="Active - User location tracking"></div>
                </div>
                <div className="flex items-center justify-between">
                  <div className="flex items-center space-x-3">
                  ...
                  <div className="w-2 h-2 bg-green-500 rounded-full animate-pulse" title="Active - Custom marker animations"></div>
                </div>
                <div className="flex items-center justify-between">
                  <div className="flex items-center space-x-3">
                    <Eye className="w-5 h-5 text-[#2DD4BF]" />
                    <span className="text-sm text-white">Intersection Observer API</span>
                  </div>
                  <div className="w-2 h-2 bg-green-500 rounded-full animate-pulse" title="Active - Deal card visibility tracking"></div>
                </div>
                <div className="flex items-center justify-between">
                  <div className="flex items-center space-x-3">
                    <Wifi className="w-5 h-5 text-[#2DD4BF]" />
                    <span className="text-sm text-white">Network Information API</span>
                  </div>
                  <div className={`w-2 h-2 ${isLowDataMode ? 'bg-yellow-500' : 'bg-green-500'} rounded-full animate-pulse`} 
                       title={isLowDataMode ? "Active - Low data mode enabled" : "Active - Normal speed connection"}></div>
                </div>
              </div>
            ...
            {/* About Section */}
            <div>
              <h4 className="text-lg font-semibold mb-4 text-white">About the Project</h4>
              <p className="text-sm text-gray-300 leading-relaxed">
                The Bengaluru Impact Visualizer bridges the gap between digital investments and real-world impact, 
                providing transparency and tangible connection to local businesses.
              </p>
            </div>
            ...
            <div>
              ...
              <div className="space-y-2">
                <div className="flex items-center space-x-3">
                  ...
                  <span className="text-sm text-white">info@bengaluruimpact.com</span>
                </div>
                <div className="flex items-center space-x-3">
          ...
          <div className="border-t border-gray-700 mt-8 pt-8 text-center">
            ...
            <p className="text-xs text-gray-500">
              * All data displayed is dummy/simulated content for demonstration purposes only
            </p>
          </div>
        </div>
      </footer>
          ...
          {selectedDeal && (
            <AnimatePresence>
              ...
              >
                <div className="flex items-center justify-between">
                  <div>
                    <h4 className="text-xl font-bold text-white">{selectedDeal.name}</h4>
                    <p className="text-gray-400">{selectedDeal.sector} • {selectedDeal.location}</p>
                  </div>
                  <div className="text-right">
                ...
                <div>
                  <div className="flex justify-between text-sm mb-2">
                    <span className="text-white">Funding Progress</span>
                    <span className="text-[#2DD4BF]">{selectedDeal.progress}%</span>
                  </div>
                ...
                <div>
                  <h5 className="font-semibold mb-2 text-white">About this Investment</h5>
                  <p className="text-gray-300 leading-relaxed">{selectedDeal.description}</p>
                </div>
                
                <div className="grid grid-cols-2 gap-4">
                  <div className="glass-morphism p-4 rounded-xl">
                    <div className="text-sm text-gray-400">Location</div>
                    <div className="font-semibold text-white">{selectedDeal.location}</div>
                    <div className="text-xs text-gray-500">
                      {selectedDeal.coordinates[0].toFixed(4)}°N, {selectedDeal.coordinates[1].toFixed(4)}°E
                    </div>
                  </div>
                  <div className="glass-morphism p-4 rounded-xl">
                    <div className="text-sm text-gray-400">Sector</div>
                    <div className="font-semibold text-white">{selectedDeal.sector}</div>
                  </div>
                </div>
                ...
                <div className="flex space-x-4">
                  ...
                  <Button variant="outline" className="glass-morphism text-white hover:bg-white/10 border-white/10">
                    Learn More
                  </Button>
                </div>
              </motion.div>
      ...
      <Dialog open={showInvestmentModal} onOpenChange={setShowInvestmentModal}>
        <DialogContent className="glass-morphism border-white/10 max-w-md">
          ...
          >
            {selectedDeal ? (
              <div className="glass-morphism p-4 rounded-xl">
                <h4 className="font-semibold text-white">{selectedDeal.name}</h4>
                <p className="text-sm text-gray-400">{selectedDeal.sector} • {selectedDeal.location}</p>
                <div className="flex justify-between mt-2">
                  <span className="text-sm text-gray-300">Current funding:</span>
                  <span className="text-[#2DD4BF] font-semibold">{formatCurrency(selectedDeal.currentFunding)}</span>
                </div>
              </div>
            ) : (
              <div className="text-center py-4">
                <p className="text-gray-300 mb-4">Choose from our available investment opportunities below.</p>
                  ...
                  {deals.map(deal => (
                    ...
                    >
                      <div className="flex justify-between items-center">
                        <div>
                          <div className="font-semibold text-white text-sm">{deal.name}</div>
                          <div className="text-xs text-gray-400">{deal.sector}</div>
                        </div>
            ...
            {selectedDeal && (
              <div className="space-y-4">
                <div>
                  <Label htmlFor="investment-amount" className="text-sm font-medium text-white">
                    Investment Amount (₹)
                  </Label>
                  <Input
                ...
                <div className="flex space-x-3">
                  <Button
                    ...
                    disabled={!investmentAmount || parseInt(investmentAmount) < 10000}
        ...
        <DialogContent className="glass-morphism border-white/10 max-w-lg">
          ...
          >
            ...
            <div className="flex items-center space-x-4">
              ...
              <div>
                <h3 className="text-lg font-semibold text-white">Rajesh Kumar</h3>
            ...
            {/* Recent Activity */}
            <div>
              <h4 className="font-semibold mb-3 text-white">Recent Investments</h4>
              <div className="space-y-2">
                <div className="glass-morphism p-3 rounded-lg">
                  <div className="flex justify-between items-center">
                    <div>
                      <div className="text-sm font-semibold text-white">Toit Brewpub</div>
                      <div className="text-xs text-gray-400">2 days ago</div>
                    </div>
                    <div className="text-[#2DD4BF] text-sm font-semibold">₹50,000</div>
                  </div>
                </div>
                <div className="glass-morphism p-3 rounded-lg">
                  <div className="flex justify-between items-center">
                    <div>
                      <div className="text-sm font-semibold text-white">NextGen Software</div>
                      <div className="text-xs text-gray-400">1 week ago</div>
                    </div>
                    <div className="text-[#2DD4BF] text-sm font-semibold">₹1,00,000</div>
                  </div>
                </div>
              </div>
            </div>
